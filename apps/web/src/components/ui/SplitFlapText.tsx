  ...
  useEffect(() => {
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const plans = targetChars
        .map((targetChar, index) => {
          const fromChar = (fromPhrase[index] as string) || ' ';
          ...
          return {
            ...
            from: fromChar,
            target: targetChar,
            sequence: buildSequence(targetChar, safeFlips, activeCharset as string),
    ...
    const scheduleNext = (delay: number) => {
      cycleTimerRef.current = window.setTimeout(() => {
        ...
        const animationDuration = animateTo(normalizedPhrases[phraseIndex] as string);
